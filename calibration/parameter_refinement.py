...
def value_function(parameters, W, obj_points, img_points):
    ...
    for i in range(M):
        ...
        for j in range(N):
            Y = np.append(Y, get_single_project_coordinates(A, W_temp, np.array([parameters[5], parameters[6]]),
                                                            (obj_points[i])[j]))
...
def jacobian_function(parameters, extrinsics, obj_points, img_points):
    ...
    for i in range(M):
        ...
        for j in range(N):
            res = np.append(res, get_single_project_coordinates(A, W, np.array([parameters[5], parameters[6]]),
                                                                (obj_points[i])[j]))
